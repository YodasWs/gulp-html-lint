...
const fs = require('fs'),
	...
	htmllint = require('@yodasws/htmllint'),
	...
	return transform((file, enc, cb) => {
		...
		htmllint(file.contents.toString(), htmlLintOptions)
			.then((issues) => {
				...
				return cb(new PluginError(PLUGIN_NAME, `Could not lint html file: ${relativeFilePath}`));
			});
	});
}
 ...
 * @param {(Function)} [writer=fancy-log] - Output writer
 * @returns {stream} gulp file stream
 */
...
function wrapError(err) {
	if (err && !(err instanceof PluginError)) {
